...
const Skills = () => {
  ...
  { 'FrontEnd Development': [ skills.find(s => s.name === 'React.js'), 
    skills.find(s => s.name === 'JavaScript'), 
    skills.find(s => s.name === 'CSS'),
    skills.find(s => s.name === 'Tailwind CSS'),
     skills.find(s => s.name === 'Html'), 
    ].filter(Boolean), 
    'Backend & APIS': [ skills.find(s => s.name === 'Java'), 
    skills.find(s => s.name === 'REST APIs'), 
    ].filter(Boolean), 
    'Tools & Other': 
    [ skills.find(s => s.name === 'Git & GitHub'),
     skills.find(s => s.name === 'Responsive Design'), skills.find(s => s.name === 'Figma'), skills.find(s => s.name === 'Vite'), 
    ].filter(Boolean), 
};
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn delay={100}>
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl lg:text-5xl  text-white mb-4">Skills & Technologies</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {Object.entries(skillCategories).map(([category, categorySkills], categoryIndex) => (
            <FadeIn key={category} delay={categoryIndex * 100}>
              <div className="relative bg-primary/10 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-300 group">
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/10">
                ...
                <div className="space-y-5">
                  {categorySkills.map((skill, skillIndex) => {
                    ...
                    return (
                      <div key={skill.id} className="space-y-2">
                        ...
                        {/* Proficiency Bar */}
                        <div className="relative h-1.5 bg-white/5 rounded-full overflow-hideed">
                  ...
                  {/* Hover Glow Effect */}
                  <div className="absolute iset-0 bg-linear-to-r from-primary/5 group-hover:to-primary/5 rounded-2xl trasnition-all duration-300 pointer-event-none"></div>
              </div>
            </FadeIn>
          ))}
        </div>
